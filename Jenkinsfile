    ...
    stages {
        ...
        stage('Push Docker Images to Nexus3') {
            steps {
                echo "${DOCKER_REPOSITORY}:${TAGS}"
                script {
                    sh 'docker tag ${DOCKER_REPOSITORY}:${TAGS} 192.168.19.15:8082/repository/docker/${DOCKER_REPOSITORY}:${TAGS}'
                    sh 'docker images'
                    sh 'docker push 192.168.19.15:8082/repository/docker/${DOCKER_REPOSITORY}:${TAGS}'
                }
            }
        }
